...
void WriteFileLines(string path, vector<string> lines, bool append = true) {
  ...
  if (!append) status = ios::in | ios::out | ios::trunc;  // overwrite
  ...
  for (auto line : lines) file_handler << line << "\n";
...
struct User {
  ...
  User() { user_id = allow_anonymous_questions = -1; }
...
struct QuestionsManager {
  ...
  void LoadDatabase() {
    ...
    for (auto &line : lines) {
      ...
      if (question.parent_question_id == -1)
        questionid_questionidsThead_to_map[question.question_id].push_back(
            question.question_id);
      else
        questionid_questionidsThead_to_map[question.parent_question_id]
    ...
    for (auto &pair :
      ...
      for (auto &question_id : pair.second) {   //  vector<Question>
        ...
        if (question.to_user_id == user.user_id) {
          if (question.parent_question_id == -1)
            user.questionid_questionidsThead_to_map[question.question_id]
                .push_back(question.question_id);
          else
            user.questionid_questionidsThead_to_map[question.parent_question_id]
                .push_back(question.question_id);
        }
      }
    }
  ...
  void PrintUserToQuestions(User &user) {
    ...
    for (auto &pair :
         user.questionid_questionidsThead_to_map) {  // pair<int,
                                                     // vector<Question>>
      for (auto &question_id : pair.second) {        //  vector<Question>
  ...
  void PrintUserFromQuestions(User &user) {
    cout << "\n";
    if (user.questions_id_from_me.size() == 0) cout << "No Questions";
  ...
  void AskQuestion(User &user, pair<int, int> to_user_pair) {
    ...
    if (question.parent_question_id == -1)
      ...
      questionid_questionidsThead_to_map[question.parent_question_id].push_back(
          question.question_id);
  }
    ...
    while (true) {
      ...
      if (choice == 1)
        ...
        if (to_user_pair.first != -1) {
          questions_manager.AskQuestion(users_manager.current_user,
                                        to_user_pair);
          questions_manager.UpdateDatabase();
        }
      } else if (choice == 6)
